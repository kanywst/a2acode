...
`Plan` is the one event the `claude` backend has to build rather than translate: Claude's task list *is* the plan and it arrives as tool calls that change one entry at a time, so `PlanTracker` accumulates it across the run. A change lands when its call has succeeded — a denied one would otherwise leave the caller reading a plan the agent never took on — and a created task's id comes back in that same result rather than in the call. `TodoWrite`, which wrote the whole list in one call, still works for older CLIs.
